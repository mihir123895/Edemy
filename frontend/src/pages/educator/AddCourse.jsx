...
const AddCourse = () => {
      ...
      setChapters(
        chapters.map((chapter) => {
          if (chapter.chapterId === chapterId) {
            chapter.chapterContent.splice(lectureIndex, 1); // Remove the lecture from the chapter
          }
          return chapter;
        })
      );
  ...
  const addLecture = () => {
    // Create a new lecture object with a unique lectureId
    const newLecture = {
      ...lectureDetails,
      lectureorder: chapters
        .find((chapter) => chapter.chapterId === currentChapterId)
        .chapterContent.length > 0
        ? chapters.find((chapter) => chapter.chapterId === currentChapterId)
            .chapterContent.slice(-1)[0].lectureorder + 1
        : 1,
      lectureId: uniqid(),
    };

    // Update the chapters state immutably
    ...
    // Close the popup and reset lecture details
    setShowPopup(false);
    setLectureDetails({
      lectureTitle: '',
      lectureDuration: '',
      ...
      const courseData = {
        ...
        courseContent: chapters.map((chapter, chapterIndex) => ({
          ...
          chapterContent: chapter.chapterContent.map((lecture, lectureIndex) => ({
            ...lecture,
            lectureOrder: lectureIndex + 1, // Ensure lectureOrder is set based on the index
          })),
        }))
