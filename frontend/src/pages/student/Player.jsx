...
const Player = () => {
  ...
  const markLecturesAsComplated = async (lectureId) =>{
    try {
                ...
                <div className={`overflow-hidden transition-all duration-300 ${openSections[index] ? "max-h-96" :"max-h-0"}`}>
                  <ul className='list-disc md:pl-10 pl-4 pr-4 py-2 text-gray-600 border-t border-gray-300'>
                    ...
                    {chapter.chapterContent.map((lecture,i)=>(
                      <li className='flex items-start gap-2 py-1' key={i}><img src={progressData && progressData.lectureCompleted.includes(playerData.lectureId) ? assets.blue_tick_icon : assets.play_icon } alt="play icon" className='w-4 h-4 mt-1' />
    ...
    playerData ? (
      <div className='md:mt-10'>
       ...
       <div className='flex justify-between items-center mt-1'>
        ...
        <button onClick={()=>markLecturesAsComplated(playerData.lectureId)} className='text-blue-600'>{progressData && progressData.lectureCompleted.includes(playerData.lectureId) ? "Completed" : "Mark Completed"}</button>
       </div>
      </div>
