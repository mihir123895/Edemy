...
useEffect(()=>{
  if(userData && courseData){
    setIsAlreadyEnrolled(userData?.enrollCourse?.includes(courseData._id) || false);
  ...
  return courseData ? (
   ...
   <div className='flex md:flex-row flex-col-reverse gap-10 relative items-start justify-between md:px-36 px-8 md:pt-30 pt-20 text-left'>
    ...
    <div className='absolute top-0 left-0 w-full section -z-1 bg-gradient-to-b from-cyan-100/70'></div>
...
<div className='flex items-center space-x-2 pt-3 pb-1 text-sm'>
        ...
        <div className='flex'>
          {[...Array(5)].map((_,i)=>(
            <img src={i < Math.floor(calculateRating(courseData)) ? assets.star : assets.star_blank} key={i} alt="" className='w-3.5 h-3.5' />
          ))}
        </div>
              ...
              <div className={`overflow-hidden transition-all duration-300 ${openSections[index] ? "max-h-96" :"max-h-0"}`}>
                <ul className='list-disc md:pl-10 pl-4 pr-4 py-2 text-gray-600 border-t border-gray-300'>
                  {chapter.chapterContent.map((lecture,i)=>(
                    <li className='flex items-start gap-2 py-1' key={i}><img src={assets.play_icon} alt="play icon" className='w-4 h-4 mt-1' />
